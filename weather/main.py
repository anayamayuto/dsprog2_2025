...
async def main(page: ft.Page):
    ...
    async def fetch_data(url: str) -> Dict:
        try:
            ...
            show_error(f"HTTPエラー: {str(e)}")
            return {}

    # 地域リストを読み込む関数
    async def load_region_list():
        try:
            progress_bar.visible = True
            page.update()

            # 気象庁のAPIから地域データを取得
            data = await fetch_data("https://www.jma.go.jp/bosai/common/const/area.json")
            if "offices" in data:
    ...
    def update_region_menu():
        ...
        for code, area in area_cache.items():
            if code in ["014030", "460040"]:  # データが存在しない地域を除外
                continue
            region_list_view.controls.append(
                ft.ListTile(
                    ...
                    on_click=lambda e, code=code: page.run_task(load_forecast, code),
                )
            )
        page.update()

    # 選択した地域の天気予報を取得する関数
    async def load_forecast(region_code: str):
        try:
        ...
        try:
            ...
            for i in range(len(weather_forecasts["timeDefines"])):
                ...
                try:
                    min_temp = temp_forecasts["areas"][0]["tempsMin"][i]
                    max_temp = temp_forecasts["areas"][0]["tempsMax"][i]
                    if min_temp == "" or min_temp is None: min_temp = "--"
                    if max_temp == "" or max_temp is None: max_temp = "--"
                except (IndexError, KeyError):
                    min_temp = max_temp = "--"
                ...
                card = ft.Card(
                    content=ft.Container(content=ft.Column(
                            controls=[
                                ...
                                ft.Row(
                                    controls=[
                                        ft.Text(f"Min: {min_temp}℃", color=ft.colors.BLUE),
                                        ft.Text("/", color=ft.colors.GREY),
                                        ft.Text(f"Max: {max_temp}℃", color=ft.colors.RED),
                                    ],
